        ...
        { /* 処理終了 */
            ...
            for (i = 0; i < num_clstr; i++)
            {
                ...
                printf("center x,y = %f %f \n",
                    center_clstr[point_clstr[i]][0]/40.0,
                    center_clstr[point_clstr[i]][1]/2.0);
...
void load_data()
{
    FILE *fp = fopen("old-faithful.txt", "r");
    int taiki, taiki_buf[1000];
    double jizoku, jizoku_buf[1000];

    int n = 0;
    while (fscanf(fp, "%lf %d", &jizoku, &taiki) != EOF)
    {
        taiki_buf[n] = taiki;
        jizoku_buf[n] = jizoku;
        n++;
    ...
    for (size_t y = 0; y < height[1]; y++)
        ...
        {
            image[1][x][y] = (unsigned char)0;
        }

    for (size_t i = 0; i < n; i++)
    {
        int x = 40 * jizoku_buf[i];
        int y = 256 - 2 * taiki_buf[i];
        image[1][x][y] = (unsigned char)255;
    }
    fclose(fp);
}
