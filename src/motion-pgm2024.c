                        ...
                        ys - y_tmp >= 0 && ys + y_tmp < height[1])
                    {
                        ...
                        for (i = -y_tmp; i <= y_tmp; i++)
                        {
                            for (j = -x_tmp; j <= x_tmp; j++)
                            {
                                ...
                                yp = ys + i;
                                sum = sum +
                                      abs(tmp_image[j + x_tmp][i + y_tmp] - image[2][xp][yp]);
                                /* 誤差＝各画素の階調値の差分の絶対値 */
                            }
                        }
...
/* してから保存する．                                            */
{
    ...
    float xave = (float)xsum / countopt;
    float yave = (float)ysum / countopt;
    printf("x, y [dot], theta = %lf %lf %lf\n", xave, yave, atan2f(yave, xave));
    printf("x, y [cm] = %lf %lf\n", xave * 8. / 100., yave * 8. / 100.);
}
