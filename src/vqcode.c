...
/* ＶＱ法で画像No.nを符号化してデータを保存 */
{
    ...
    for(i=0;i<NUM;i++){
        ...
        for(t=0;t<BXY;t++)
            for(s=0;s<BXY;s++)
                rblock[i][s][t] = image[n][BXY*x+s][BXY*y+t];
        flag[x][y] = 1;
    }
    /* k-平均法によるクラスタリング */
    ...
    do{
        ...
        for(j=0;j<NY;j++){
            for(i=0;i<NX;i++){
                min = distance(0,n,i,j);
                minnum = 0;
                for(k=1;k<NUM;k++){
                    dist = distance(k,n,i,j);
                    if ( dist < min ){
                        min = dist;  minnum = k;
                    }
                }
    ...
    for(i=0;i<NUM;i++){
        for(y=0;y<BXY;y++){
            for(x=0;x<BXY;x++){
                fprintf(fp,"%d",rblock[i][x][y]);
                if (x != BXY-1) fprintf(fp,",");
            }
            fprintf(fp,"\n");
        }
    }
