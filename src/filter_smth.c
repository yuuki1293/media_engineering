...
int Cij[3][3] =      /* Cij[j][i] の順になることに注意 */
   {{ 1,  1,  1},    /* j=-1 で i=-1,i=0,i=+1 と変化 */
    { 1,  2,  1},    /* j= 0 で i=-1,i=0,i=+1 と変化 */
    { 1,  1,  1}};   /* j=+1 で i=-1,i=0,i=+1 と変化 */
double K[] = {1./9., 1./10.}; /* 最後に掛ける係数 */

int calc( int n, int x, int y, double k );
void filtering( int n1, int n2, int kn );

int main(void)
{
    load_image( 0, "" );   /* ファイル → 画像No.0 */
    filtering( 0, 1 , 0);     /* No.0->フィルタ->No.1 */
    filtering( 1, 2, 1);
