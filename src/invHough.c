...
/* 結果を image2[y][x] に描く                                    */
{
    ...
    rho_max = 1.0 / 1.5 * width[2] + 1.0 / 1.5 * height[2];
    ...
    do {
        ...
        for ( y = 0; y < MAX_RHO;  y ++ )
            for ( x = 0; x < MAX_THETA; x ++ )
                if ( image[1][x][y] >= threshold ) counter ++;
...
void check_data( )
{
    if ( width[1] != MAX_THETA || height[1] != MAX_RHO ){
...
main( )
{
    ...
    load_image(1,"" );  /* θρ平面のデータを image1 へ */
    check_data( );  /* データのチェック */
    inverse_Hough_transform( );  /* Hough逆変換して直線抽出 */
    save_image(2,"" );  /* image2 を保存 */
    return 0;
}
